...
const BreadcrumbItem: React.FC<Props> = ({ children, ...props }) => {
	return (
		<li
			className="text-sm px-2.5 transition duration-200 ease-in first:ps-0 last:pe-0 hover:text-secondary"
			{...props}
		>
			{children}
		</li>
...
export const BreadcrumbItems = (props: any) => {
	...
	return (
		<div className="chawkbazarBreadcrumb flex items-center text-info font-text">
...
const Breadcrumb: React.FC<{ separator?: string }> = ({ separator = "/" }) => {
	...
	return (
		<BreadcrumbItems separator={separator}>
			<ActiveLink href={"/"} activeClassName="font-semibold text-secondary">
			...
			{breadcrumbs?.map((breadcrumb: any) => (
				<ActiveLink
					...
					activeClassName="font-semibold text-secondary"
					key={breadcrumb.href}
				>
