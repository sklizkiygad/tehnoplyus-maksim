...
const TestimonialCard: React.FC<Props> = ({ item }) => {
  return (
    <div className="bg-base-200 rounded-md p-8 sm:p-6 md:p-8 transition duration-300 ease-in-out w-[92%] md:w-[96.5%] lg:w-full mx-auto md:mx-0">
      ...
      <div className="inline-grid grid-cols-5 gap-1.5 mt-5">
        {Array.from({ length: item.rating }).map((_, idx) => (
          <StarIcon key={idx} className="fill-accent" />
        ))}
