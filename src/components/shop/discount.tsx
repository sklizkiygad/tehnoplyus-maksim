...
const ShopDiscount: React.FC = () => {
	...
	return (
		<div
			className={`flex justify-center relative bg-base-200 transition duration-200 ease-in ${
				status === true ? "h-0.5" : "py-4"
			}`}
		>
			<Container className={status === true ? "opacity-0 invisible" : "w-full"}>
				<div className="relative text-center text-accent font-text text-xs md:text-sm leading-6 md:leading-7 px-8">
					{t("text-discount")} &nbsp;
